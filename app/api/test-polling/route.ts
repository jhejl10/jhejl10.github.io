...
export async function POST(request: Request) {
  try {
    ...
    if (presenceStatus) {
      ...
      console.log("=== MANUAL TEST PRESENCE UPDATE (POLLING) ===")
    ...
    if (statusMessage !== undefined) {
      ...
      console.log("=== MANUAL TEST STATUS MESSAGE UPDATE (POLLING) ===")
    ...
    return NextResponse.json({
      success: true,
      message: "Test updates sent via polling",
      timestamp: new Date().toISOString(),
    })
  } catch (error) {
...
export async function GET() {
  return NextResponse.json({
    message: "Send a POST request to test polling updates",
    example: {
